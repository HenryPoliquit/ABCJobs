		...
		<div class="flex-col gap-20">
			...
			<tbody class="pFont">
				<c:if test="${not empty threads}">
					<c:forEach items="${threads}" var="p" varStatus="status">
						<tr>
							<td>${status.count}</td>
							<td>${p.name}</td>
							<td>${p.description}</td>
							...
							<td>
								<div class="flex center gap-10">
									<!-- Update -->
									<button id="createThread" class="btnAnimation material-icons">edit</button>
			<!-- Post Modal -->
			...
			<h3 class="modal-heading">Edit Thread</h3>
			<sf:form id="createThreadForm" class="align-center flex-col form"
				onsubmit="validateCreateThread(event)" action="edit_thread?tId=${p.id}"
				...
				<div class="input-group">
					<input required="true" type="text" name="name" path="name"
						...
						value="${p.name}" /> <label class="user-label">Name</label>
				</div>
				<div class="input-group">
			...
			<dialog id="deleteModal" class="modal">
				...
				<c:if test="${empty threads}">
					<tr>
						<td colspan="6">No threads made yet</td>
					</tr>
				</c:if>
			</tbody>
				</table>			
